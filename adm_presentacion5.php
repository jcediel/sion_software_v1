		...
		if($_POST["aceptar"]!="Descartar"){
			...
			if (isset($_FILES["foto"])){
				if (is_uploaded_file($_FILES["foto"]["tmp_name"])){
					...
					if(move_uploaded_file($_FILES["foto"]["tmp_name"],WEBE.DOCPAC.$archivo)){
			...
			switch ($_POST["operacion"]) {
				case 'ADDDETALLE':
					...
					if ($tabla_validacion=$bd1->sub_tuplas($sql_validacion)){
						...
						$sql="INSERT INTO d_aut_dom (id_m_aut_dom,resp_d_aut_dom,cups,procedimiento,cantidad,finicio,ffinal,
																				 estado_d_aut_dom,intervalo,temporalidad,dosis,frecuencia,succion)
									 VALUES ('".$_POST["idm"]."','".$_SESSION["AUT"]["id_user"]."','".$_POST["cups"]."', '".$_POST["procedimiento"]."',
									'".$_POST["cantidad"]."','".$_POST["finicio"]."','".$_POST["ffinal"]."',
									'1', '".$_POST["intervalo"]."', '".$_POST["temporalidad"]."','".$_POST["dosis"]."','".$_POST["frecuencia"]."','".$_POST["succion"]."')";
						$subtitulo="El Procedimiento $procedimiento ";
			...
			case 'ADDAUTORIZACION':
				$sql1="UPDATE d_aut_dom SET resp_inicial_num='".$_SESSION["AUT"]["id_user"]."',num_aut_externa='".$_POST["num_aut_externa"]."'
				WHERE id_d_aut_dom='".$_POST["id_d_aut_dom"]."' ";
				$subtitulo2="Se ha realizado la asignación de un numero de autorización para el servicio ";
			break;
...
if (isset($_GET["mante"])){					///nivel 2
	switch ($_GET["mante"]) {
		...
		case 'MASTER':
			$idadm=$_REQUEST['idadm'];
			$sql="SELECT a.tdoc_pac,doc_pac,nom1,nom2,ape1,ape2,nom_completo,
                   b.fingreso_hosp,id_adm_hosp,
                   c.nom_eps,c.id_eps ideps
            FROM pacientes a INNER JOIN adm_hospitalario b on a.id_paciente=b.id_paciente
                             INNER JOIN eps c on c.id_eps=b.id_eps
            WHERE id_adm_hosp=$idadm";
			$color="yellow";
			$boton="Agregar plan principal";
			$atributo1=' readonly="readonly"';
			$atributo2='';
			$atributo3='';
			$opcion=$_REQUEST['opcion'];
		...
		case 'ADDAUTORIZACION':
			...
			$sql="SELECT a.id_m_aut_dom, a.id_adm_hosp id, id_user, tipo_paciente, zona_paciente, ips_ordena, medico_ordena, estado_p_principal,
						...
						//echo $sql;
			$color="yellow";
			$boton="Asignar Autorizacion";
			$atributo1=' readonly="readonly"';
			$atributo2='';
			$atributo3='';
			$opcion=$_REQUEST['opcion'];
			$doc=$_REQUEST['doc'];
			$form1='formulariosDOM/autorizacion/add_autorizacion.php';
			$subtitulo='Asignación de autorización a procedimiento';
		break;
		case 'ACUDIENTE':
			$idadm=$_REQUEST['idadm'];
		...
		case 'ADDBARRIO':
			...
			$opcion=$_REQUEST['opcion'];
			$doc=$_REQUEST['docc'];
			$form1='formulariosDOM/autorizacion/copy_procedimiento.php';
			$subtitulo='Arrastre de procedimiento anterior';
			break;
			case 'EDITJEFE':
			$idpac=$_REQUEST['idpac'];
			$sql="SELECT id_paciente,doc_pac,nom1,nom2,ape1,ape2,nom_completo
										...
										if ($tablam=$bd1->sub_tuplas($sqlm)){
											foreach ($tablam as $filam) {
																					...
																					if ($tabla_m=$bd1->sub_tuplas($sql_m)){
																						foreach ($tabla_m as $fila_m) {
																								echo'<article>
																											...
																											<p><strong>IPS remitente: </strong><b class="lead">'.$fila_m['nombre'].'</b></p>
												...
												$sql_d="SELECT id_d_aut_dom, freg, cups, procedimiento,
																			 cantidad,intervalo,temporalidad, finicio, ffinal, num_aut_externa,resp_inicial_num, estado_d_aut_dom
																FROM d_aut_dom
																			...
																			echo'<tr>
																			      <td>
																				      ...
																				      <p>'.$fila_d['cups'].' | '.$fila_d['procedimiento'].'</p>';
																							$tipo_pac=$filam['tipo_paciente'];
																							if ($tipo_pac==2) {
																								echo '<p><button type="button" class="btn btn-info" data-toggle="modal" data-target="#extender_'.$fila_d['id_d_aut_dom'].'"><span class="fa fa-clock"></span> Extender<br>Procedimiento</button></p>
																								<div id="extender_'.$fila_d['id_d_aut_dom'].'" class="modal fade" role="dialog">
																									<div class="modal-dialog">

																										<div class="modal-content">
																											<div class="modal-header">
																												<button type="button" class="close" data-dismiss="modal">&times;</button>
																												<h4 class="modal-title">Extension de fecha en procedimiento a paciente puntual</h4>
																											</div>
																											<div class="modal-body">
																											<form action="Funcion_base/extender_prod_dom.php" method="POST">
																												<section class="panel-body">
																													<article class="col-md-12">
																														<label>Seleccione justificacion de ampliación:</label>
																														<select class="form-control" name="justificacion_ext">
																															<option values="1">Hospitalizacion</option>
																															<option values="2">No tenia enfermeria</option>
																															<option values="3">Profesional sin asignar</option>
																															<option values="4">No cumplimiento en evoluciones</option>
																														</select>
																													</article>
																													<article class="col-md-12">
																														<label>Nueva Fecha:</label>
																														<input type="date" class="form-control" name="new_fecha" value="'.$fila_d['ffinal'].'">

																														<input type="hidden" name="resp" value="'.$_SESSION['AUT']['id_user'].'">
																														<input type="hidden" name="id" value="'.$fila_d['id_d_aut_dom'].'">
																								...
																								echo '<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=COPYPROD&idd='.$fila_d["id_d_aut_dom"].'&doc='.$_GET["doc"].'">
																									 <button type="button" class="btn btn-info" ><span class="fa fa-copy"></span> Copiar<br>Procedimiento
																									 </button></a></p>
																										';
																								...
																								if ($tabla_evoluciones=$bd1->sub_tuplas($sql_evoluciones)){
																									foreach ($tabla_evoluciones as $fila_evoluciones) {
																										...
																										if ($c > 0) {
																											...
																											echo'<p><button type="button" class="btn btn-info btn-lg" data-toggle="modal" data-target="#myModal">Open Modal</button></p>
																													 ';
																											include('');
																										}else {
																							...
																							echo'<section class="">';
																									 $tipo_paciente=$filam['tipo_paciente'];
																			        ...
																			        if ($tabla_prof=$bd1->sub_tuplas($sql_prof)){
																			          ...
																			          echo'	<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=ADDPROFESIONAL&idd='.$fila_d['id_d_aut_dom'].'&doc='.$filap["doc_pac"].'&nc='.$filap['nom_completo'].'&servicio='.$fila_adm["tipo_servicio"].'&idadm='.$fila_adm["id_adm_hosp"].'">
																			            ...
																			            <span class="fa fa-edit"></span> Asignar Profesional</button>
																							...
																							$autorizacion=$fila_d['num_aut_externa'];
																							if ($autorizacion=='') {
																								echo'<p><a href="'.PROGRAMA.'?opcion='.$_REQUEST["opcion"].'&mante=ADDAUTORIZACION&idd='.$fila_d['id_d_aut_dom'].'&doc='.$filap["doc_pac"].'&nc='.$filap['nom_completo'].'&servicio='.$fila_adm["tipo_servicio"].'&idadm='.$fila_adm["id_adm_hosp"].'">
																									<button type="button" class="btn btn-primary" >
																									<span class="fa fa-hashtag"></span> Asignar Autorización</button>
																									</a></p>';
																							}else {
																								echo'<p>
																											Este procedimiento cuenta con el numero de Autorizacion: <strong>'.$fila_d['num_aut_externa'].'</strong>
																										 </p>';
																							}
																			  echo'</td>
																			  </tr>';
